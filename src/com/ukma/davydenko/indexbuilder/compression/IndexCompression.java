...
public class IndexCompression {
   ...
   */
	public static void gapEncode(int[] inputDocIdsOutputGaps) {
		...
		for (int i = 0; i < inputDocIdsOutputGaps.length; i++) {
			int currentDocId = inputDocIdsOutputGaps[i];
			inputDocIdsOutputGaps[i] -= previousDocId;
   ...
   */
	public static void gapDecode(int[] inputGapsOutputDocIds) {
		int previousDocId = 0;
		for (int i = 0; i < inputGapsOutputDocIds.length; i++) {
			inputGapsOutputDocIds[i] += previousDocId;
			previousDocId = inputGapsOutputDocIds[i];
		}
	}
   ...
   */
	public static int VBEncodeInteger(int gap, byte[] outputVBCode) {
		...
		for (int i = 0; i < numBytes; i++) {
			outputVBCode[i] = bytes.get(numBytes - 1 - i);
		}
   ...
   */
  public static List<Integer> VBDecodeInteger(byte[] inputVBCode) {
  ...
  public static boolean testVBDecodeInteger(boolean debugPrint) {
    ...
    result = VBDecodeInteger(encodedInt);
    if (!equalInts(result.get(0), 5, debugPrint, "Decoding 0x85")) {
      success = false;
    }

    encodedInt[0] = (byte) 0x80;
    result = VBDecodeInteger(encodedInt);
    if (!equalInts(result.get(0), 0, debugPrint, "Decoding 0x80")) {
      success = false;
    }
    ...
    result = VBDecodeInteger(encodedInt);
    if (!equalInts(result.get(0), 824, debugPrint, "Decoding 0x6B8")) {
      success = false;
    }
...
//    }
  ...
  public static int[] createLecurePostingsList() {
	  int[] lecturePostingsList = {
			  824, 829, 215406
	  };
	  return lecturePostingsList;
  }
  ...
  public static void runCompressionTests() throws IOException {
    ...
    compressionTestForPostingsList(createLecurePostingsList(), "lecture");
  }
